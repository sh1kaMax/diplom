...
namespace TestV1089_positive_1
{
  namespace TestV1089_positive
  {
    std::queue<int> queue;
    std::mutex mtx;
    ...
    void consumer()
    {
      while (true)
      {
...

        {
          ...
          if (cond.wait_for(   lck, std::chrono::seconds(10)) 
                            == std::cv_status::timeout) // FLAW
          {
            break;
          }
    ...
    void ExploitFlaw(std::ifstream &fin, std::istringstream &sin)
    {
