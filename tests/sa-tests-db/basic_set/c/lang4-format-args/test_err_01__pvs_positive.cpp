...
#define length(cont) ( sizeof(cont) / sizeof(cont[0]) )

size_t my_read(const char *res, char *buf, size_t max_len);

void Err13(FILE *file)
{
  char buffer[1001];
  size_t len;
  while ((len = my_read("some_resource_name", buffer, length(buffer) - 1)) > 0)
  {
    buffer[len] = 0;
    fprintf(file, buffer); // FLAW
  }
}